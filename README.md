...
After the upgrade, `update` automatically reapplies any exporters whose installed plugin version is older than the new clw-auth version (or missing a version marker entirely). This keeps the OpenCode plugin and OpenClaw profiles in sync with the latest templates without requiring a manual `clw-auth export` run.

`--check` exit codes: `0` up to date or ahead of upstream, `10` update available or exporter reapply needed, `2` error. Useful in shell prompts and CI freshness gates:
...
Both `clw-auth --version` (concise one-liner) and `clw-auth version` (full snapshot) are supported. The `version` subcommand reports the package version, git commit and tag, commit timestamp, Node.js version, platform, install directory, data directory, and the freshness of every configured exporter — useful for issue reports and CI introspection. Use `--json` for a stable machine-readable shape.
...
# Silent mode (no stdout/stderr) — used by the OpenCode plugin's hybrid refresh.
...
- The generated plugin embeds a `// clw-auth-plugin-meta:` header on its first line so `clw-auth version` and `clw-auth update` can detect when the plugin was generated by an older clw-auth version and reapply it automatically
- The plugin delegates OAuth refresh to `clw-auth ensure-fresh --silent` before falling back to its own inline refresh logic, eliminating race conditions with cron or other concurrent refresh callers
